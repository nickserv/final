...
### Server Runner
Final can generate callbacks for Node's `http.Server` class, allowing you to run web servers wrapping `Command`s. You can also embed `Command`s in larger Node web apps.
```javascript
var final = require('final')
var command = new Adder()
new final.Server(command).run()
```
Here, Final starts a web server at `localhost:3000` that wraps your `Command`. You can call it with HTTP requests like `GET localhost:3000?first=1&second=2`, and you will get a plain text response with the result.
